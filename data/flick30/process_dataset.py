...
if __name__ == "__main__":
    build_vocab = False            # If True, the vocabulary will be built again and saved. If False, the last vocabulary will be loaded.
    ...
    if build_vocab:
        ...
        m = 1
        ids = [f[:-4] for f in os.listdir(data_folder + 'data/Sentences') if f.endswith('.txt')]
        N = len(ids)
        for img_n, img_id in enumerate(ids):
            print("\rImg {}/{}".format(img_n, N), end="")
            sentence_path = data_folder + 'data/Sentences/' + img_id + '.txt'
    ...
    if generate_annotations or generate_visual:
        ...
        for img_n, img_id in enumerate(ids):
            ...
            annotationData = get_annotations(data_folder + 'data/Annotations/'+img_id+'.xml')
            ...
            if generate_annotations:
                ...
                with open(data_folder + 'annotation_data/'+img_id+'.pkl', 'wb') as f:
                    pickle.dump(image_annotations, f, protocol=pickle.HIGHEST_PROTOCOL)
                if img_n % 100 == 0:
                    print(f"\rImage {img_n}/{N} - Phrases: {phrase_count}", end="")
